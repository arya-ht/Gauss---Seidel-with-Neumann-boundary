...
{
    return (fabs(approx - exact) / fabs(approx)) * 10.0;
}
void print(double U[SIZE_T + 1][SIZE_X + 1])
{
    int n, j;
...
void Gauss_S()
{
    int n, k, i, t, x, tol;
    //init values
    double a = 1.0 + 2.0*lambda, b = -lambda, c = -lambda,u[SIZE_T + 1][SIZE_X + 1],
    ...
    for (k = 0; k <= SIZE_T; k++)
    {
        for (i = 0; i <= SIZE_X; i++)
        {
            u[k][i] = cos(i*(n - 0.5)*dx);
        }
    }
    //set right/left bounds
    for (n = 0; n <= SIZE_T; n++)
    {   u[n][0] = (u[n-1][0] -2*( b*u[n][0] - c*u[n][1])) / a;
        //u[n][0] = 0; -neumann boundary
        u[n][SIZE_X] = 0;
    }
